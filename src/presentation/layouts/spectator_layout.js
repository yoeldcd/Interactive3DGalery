...
export class SpectatorLayout {
    ...
    static render({ galleryName = 'Galería Virtual', hasVisitedOnce = false }) {
        ...
        return `
                    <div class="w-full h-full relative pointer-events-none flex flex-col justify-between p-6">
                        <div class="flex justify-between items-center w-full pointer-events-auto relative z-30">
                            <div class="bg-slate-900/80 backdrop-blur-md px-4 py-2 rounded-2xl border border-slate-700/60 shadow-lg flex items-center gap-3">
                                <div class="w-2.5 h-2.5 rounded-full bg-emerald-400 animate-pulse"></div>
                                <span class="text-xs font-bold text-white uppercase tracking-wider">${galleryName}</span>
                                <span class="text-xs text-slate-400">• Modo Espectador</span>
                            </div>

                            <div class="flex items-center gap-2">
                                <button data-action="setMode" data-mode="EDITOR" class="px-4 py-2 bg-slate-900/85 hover:bg-slate-800 text-slate-200 hover:text-white border border-slate-700/60 rounded-xl backdrop-blur-md transition text-xs font-bold flex items-center gap-2 shadow-lg hover:scale-105 active:scale-95 cursor-pointer">
                                ...
                                <button data-action="setMode" data-mode="START" class="px-4 py-2 bg-slate-900/85 hover:bg-slate-800 text-slate-200 hover:text-white border border-slate-700/60 rounded-xl backdrop-blur-md transition text-xs font-bold flex items-center gap-2 shadow-lg hover:scale-105 active:scale-95 cursor-pointer">
                        ...
                        <div id="click-to-play-overlay" class="absolute inset-0 z-20 flex items-center justify-center pointer-events-auto bg-black/40 backdrop-blur-[2px]">
                            <div class="bg-slate-950/95 backdrop-blur-md px-8 py-7 rounded-2xl border border-amber-400/30 text-center shadow-2xl max-w-md w-full mx-4 transform transition">
                                <h3 class="text-lg font-extrabold text-white mb-1 tracking-wide">Guía de Exploración</h3>
                                <p class="text-xs text-slate-400 mb-4 leading-relaxed">Controla el desplazamiento y la orientación con las teclas:</p>
                                ...
                                <p class="text-[11px] text-slate-400 mt-2 mb-4 font-medium">Haz clic en la pantalla o presiona <kbd class="px-1.5 py-0.5 bg-slate-800 rounded text-amber-400 font-mono border border-slate-700">ESC</kbd> para alternar el control del ratón</p>
                        ...
                        <div class="flex justify-center items-end w-full relative z-30 pointer-events-none">
                            <div id="info-hud" style="width: 50dvw; height: 200px;" class="bg-black/35 backdrop-blur-md p-5 sm:p-6 rounded-2xl border border-white/20 text-white shadow-2xl opacity-0 transform translate-y-4 transition-all duration-300 pointer-events-none flex flex-col justify-start overflow-hidden">
                                <div class="flex items-center gap-2.5 mb-2.5 flex-shrink-0">
                                    <span class="w-2.5 h-2.5 rounded-full bg-amber-400 animate-pulse shadow-[0_0_8px_#facc15]"></span>
                                    <span class="text-xs sm:text-sm font-black text-amber-400 uppercase tracking-widest drop-shadow-sm">Descripción de la Obra</span>
                                </div>
                                <div class="flex-1 overflow-hidden">
                                    <p id="hud-desc" class="text-sm sm:text-base md:text-lg text-white font-medium leading-relaxed drop-shadow"></p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    /**
     * Vincula los listeners de eventos para iniciar el recorrido PointerLock.
     * @param {object} options
